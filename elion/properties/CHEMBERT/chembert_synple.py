...
class SMILES_Dataset(Dataset):
    '''
    A Dataset of SMILES with Price and Parent IDs.
    '''
    ...
    def __getitem__(self, idx):
        ...
        # Mapping price to 'smiles_bert_label' to maintain model compatibility
        # Included 'smiles_bert_id' for the parent_id
        output = {
        ...
        # Note: This assumes parent_id is numeric. 
        # If bb3_parent_id contains strings, remove it from this dict comprehension 
        # or convert it to an integer index.
...
def SMILES_Dataset_from_file(smiles_file):
    ...
    text = pd.DataFrame()
    if has_header:
        text = pd.read_csv(smiles_file, header=0, names=['bb3_parent_id','SMILES','Price'])
    else:
        text = pd.read_csv(smiles_file, names=['bb3_parent_id','SMILES','Price'])

    parent_id_list = np.asarray(text['bb3_parent_id'].values)
    smiles_list = np.asarray(text['SMILES'].values)
    price_list  = np.asarray(text['Price'].values)
