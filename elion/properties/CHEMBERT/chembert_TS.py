...
class SMILES_Dataset(Dataset):
    '''
    A Dataset of SMILES with Score (formerly Price) and Name IDs.
    '''
    def __init__(self, parent_id_list, smiles_list, price_list, vocab=Vocab(), seq_len=256):
        self.vocab = vocab
        ...
        self.smiles_dataset = []
        self.adj_dataset = []
        self.parent_ids = parent_id_list
        
        # Handle prices/scores
        if len(price_list) == 0:
            price_list = np.zeros_like(smiles_list, dtype=np.float64)
        self.prices = price_list.reshape(-1, 1)
    ...
    def __getitem__(self, idx):
        ...
        parent_id  = self.parent_ids[idx] # This is now a string (e.g. '44691894_49274833')
        ...
        output = {
            "smiles_bert_input": smiles_bert_input,
            "smiles_bert_label": price,  
            "smiles_bert_adj_mask": smiles_bert_adj_mask,
            "smiles_bert_adjmat": smiles_bert_adjmat,
            "smiles_bert_id": parent_id
        }

        # FIX: Do not convert parent_id to tensor because it is a string.
        final_output = {}
        for key, value in output.items():
            if key == "smiles_bert_id":
                final_output[key] = value # Keep as string
            else:
                final_output[key] = torch.tensor(value)
...
class chembert_model:
    ...
    def predict(self,dataset):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader):
                # data = {key:value.to(device) for key, value in data.items()}
                data = {key:value.to(device) if key != "smiles_bert_id" else value for key, value in data.items()}
                position_num = torch.arange(256).repeat(data["smiles_bert_input"].size(0),1).to(device)
                output = model.forward(data["smiles_bert_input"], position_num,
                                    adj_mask=data["smiles_bert_adj_mask"],
                                    adj_mat=data["smiles_bert_adjmat"])
...
def SMILES_Dataset_from_file(smiles_file):
    """
    Reads SMILES from a file and returns a Dataset object.
    Adapted for columns: score, SMILES, Name
    """
    import pandas as pd
    import csv
    import numpy as np
    ...
    with open(smiles_file, 'r') as smi_file:
        try:
            ...
            # Fallback if sniffer fails, assume header exists based on your format
            has_header = True
        smi_file.seek(0)

    text = pd.DataFrame()
    if has_header:
        # Reads the file using the existing header (score, SMILES, Name)
        text = pd.read_csv(smiles_file, header=0)
    else:
        # Fallback manual naming if header is missing
        text = pd.read_csv(smiles_file, names=['score', 'SMILES', 'Name'])

    # Map the new columns to the variables expected by the Dataset class
    # 'Name' acts as the parent_id
    parent_id_list = np.asarray(text['Name'].values)
    
    # 'SMILES' remains the same
    smiles_list = np.asarray(text['SMILES'].values)
    
    # 'score' acts as the price/label
    price_list  = np.asarray(text['score'].values)

    print(f"Read {len(text)} molecules from file {smiles_file}.")
    return SMILES_Dataset(parent_id_list, smiles_list, price_list)
