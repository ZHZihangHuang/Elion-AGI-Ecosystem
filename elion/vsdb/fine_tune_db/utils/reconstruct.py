...
def reconstruct_from_generated_with_bond(xyz, atomic_nums, bond_index, bond_type, check_validity=True):
    ...
    mol = rd_mol.GetMol()
    directory = '/blue/lic/huangzihang/repos/DecompDiff/generated_ligands/'
    file_count = len([f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f))])
    ligand_idx = file_count + 1
    if check_validity:
        try:
            # print(mol)
        ...
        print('mol: %s' % mol)
        try:
            print('checking using Chem.SanitizeMol again')
            Chem.SanitizeMol(mol)
            print('it pass the Chem.SanitizeMol!')
        except Exception as e:
            raise MolReconsError()
            # return None

    w = Chem.SDWriter('/blue/lic/huangzihang/repos/DecompDiff/generated_ligands/%s.sdf' % ligand_idx)
    w.write(mol)
    w.close()
    # check valid
