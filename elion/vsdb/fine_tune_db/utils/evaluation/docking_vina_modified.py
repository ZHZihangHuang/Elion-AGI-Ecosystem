...
class PrepLig(object):
    ...
    def get_pdbqt(self, lig_pdbqt=None):
        try:
            ...
            raise ValueError(f"Failed to generate PDBQT: {str(e)}")
...
class PrepProt(object): 
    ...
    def addH(self, prot_pqr):  # call pdb2pqr
        print('---------------------------debug41')
        sdf_match = re.search(r'(.*).sdf', self.prot)
        if sdf_match:
            self.prot = self.prot.replace('.sdf', '.pdb')
        self.prot_pqr = prot_pqr
        print(['pdb2pqr30','--ff=AMBER',self.prot, self.prot_pqr])
    ...
    def get_pdbqt(self, prot_pdbqt, prot_pqr):
        print('---------------------------debug44')
        self.prot_pqr = prot_pqr
        prepare_receptor = os.path.join(AutoDockTools.__path__[0], 'Utilities24/prepare_receptor4.py')
        print(['python3', prepare_receptor, '-r', self.prot_pqr, '-o', prot_pdbqt])
        ...
        try:
            with open(lig_pdbqt, 'r') as f:
                ...
                if 'ROOT' not in content or 'ENDROOT' not in content:
                    raise ValueError(f"Invalid PDBQT file {lig_pdbqt}: Missing ROOT/ENDROOT tags")
        except Exception as e:
            raise ValueError(f"Failed to parse PDBQT file {lig_pdbqt}: {str(e)}")

    def _max_min_pdb(self, pdb, buffer):
        print('-----------------debug132 buffer: %s' % buffer)
        with open(pdb, 'r') as f: 
            lines = [l for l in f.readlines() if l.startswith('ATOM') or l.startswith('HETATM')]
            if not lines:
                raise ValueError(f"No ATOM or HETATM lines found in {pdb}")
        ...
        try:
            ...
            if not save_pose: 
                ...
                if mode == 'score_only': 
                    ...
                    raise ValueError(f"Invalid mode for pose: {mode}")
                return score, pose
        except Exception as e:
            raise ValueError(f"Docking failed for {self.lig_pdbqt}: {str(e)}")
...
class VinaDockingTask(BaseDockingTask):
    ...
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', **kwargs):
        ...
        if 'lmdb' in protein_root:
            ...
            protein_fn = os.path.join(
                os.path.dirname(ligand_filename),
                os.path.basename(ligand_filename.replace('_ligand.sdf', '_protein.pdb'))
            )
            protein_path = os.path.join(protein_root, protein_fn)
        print(os.path.basename(ligand_filename))
        print(os.path.basename(ligand_filename.replace('_ligand.sdf', '_protein.pdb')))
    ...
    def __init__(self, protein_path, ligand_rdmol, tmp_dir='./tmp', use_uff=True, center=None,
        ...
        caller_name = caller_frame.f_code.co_name
        caller_module = inspect.getmodule(caller_frame).__name__
        print(f"The caller function is '{caller_name}' in module '{caller_module}' (current in docking_vina.py 203)")

        print(self.tmp_dir)
        print(self.ligand_id + '.sdf')
        print(self.receptor_path)
    ...
    def run(self, mode='dock', exhaustiveness=8):
        ...
        print('os.path.exists(protein_pqr): %s' % os.path.exists(protein_pqr))
        print('os.path.exists(protein_pdbqt): %s' % os.path.exists(protein_pdbqt))
